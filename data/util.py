...
def read_image(path, dtype=np.float32, color=True):
    """      
    path  : image
    color : True, return three channels and RGB order
          : False, return a grayscale image
    """
    f = Image.open(path)
    try:
        if color:
...
def flip_bbox(bbox, size, y_flip=False, x_flip=False):
    """
    bbox : see before
    size (tuple): 2 values before resized, (height, width)
    y_flip (bool): 
    x_flip (bool): 
    Return: ~numpy.ndarray
    """
...
def crop_bbox(bbox, y_slice=None, x_slice=None,
    ...
    allow_outside_center (bool): False: bounding boxes whose centers are outside 
                                        of the cropped area are removed. 
    return_param (bool): True : returns indices of kept bounding boxes.
